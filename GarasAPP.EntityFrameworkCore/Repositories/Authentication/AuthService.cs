...
namespace GarasAPP.EntityFrameworkCore.Repositories.Authentication
{
    public class AuthRepository : BaseRepository<User, long>, IAuthRepository
    {
        ...
        public AuthRepository(ApplicationDbContext context, IOptions<JWT> jwt, IHostingEnvironment Environment, IHttpContextAccessor httpContextAccessor) : base(context)
        {
            _context = context;
        ...
        public async Task<BaseResponseWithData<AuthModel>> GetTokenAsync(LoginRequestModel model)
        {
            ...
            try
            {
                ...
                {
                    Response.Result = false;
                    Response.Errors.Add(new Error { code = "E-1", message = "Invalid Email or password" });
                    return Response;
                }
                if (_user.Active != true)
                {
                    Response.Result = false;
                    Response.Errors.Add(new Error { code = "E-1", message = "This Email was not active " });
                    return Response;
                }
        ...
        public async Task<BaseResponseWithData<AuthModel>> GetUserDataAsync(string ApplicationUserId)
        {
            ...
            try
            {
                ...
                {
                    Response.Result = false;
                    Response.Errors.Add(new Error { code = "E-2", message = "Invalid User" });
                    return Response;
                }
                long uId = 0;
